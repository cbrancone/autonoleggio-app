...
# =========================================================
with tab_nuovo:
    st.subheader("➕ Registra Nuovo Noleggio (Seleziona da Flotta Disponibile)")

    if not df.empty and COL_STATO in df.columns:
        # Filtra solo i veicoli attualmente disponibili
        df_disponibili = df[df[COL_STATO] == "Disponibile"]

        if df_disponibili.empty:
            st.warning("⚠️ Al momento non ci sono veicoli con stato 'Disponibile' da poter noleggiare.")
        else:
            # Crea le opzioni per il menu a tendina
            ...
            with st.form("form_nuovo_noleggio"):
                auto_scelta = st.selectbox("Seleziona Veicolo Disponibile *", opzioni_auto)
                
                c1, c2 = st.columns(2)
                with c1:
                    cliente_nuovo = st.text_input("Nome Cliente *", placeholder="Inserisci nome e cognome")
                    data_inizio_noleggio = st.date_input("Data Inizio Noleggio *", date.today())
                with c2:
                    data_fine_noleggio = st.date_input("Data Fine Noleggio *", date.today())
                    note_noleggio = st.text_area("Note Noleggio", placeholder="Eventuali annotazioni...")

                submit_noleggio = st.form_submit_button("💾 Conferma e Avvia Noleggio", type="primary")

                if submit_noleggio:
                    if not cliente_nuovo.strip():
                        st.error("Inserisci il nome del cliente.")
                    else:
                        targa_scelta = auto_scelta.split(" - ")[0]
                        try:
                            df_agg = formatta_date_df(df)
                            ...
                            if len(idx) > 0:
                                i = idx[0]
                                # Recupera il prezzo giornaliero del veicolo scelto
                                ...
                                # Calcolo giorni e costo totale
                                giorni = (data_fine_noleggio - data_inizio_noleggio).days
                                giorni = 1 if giorni < 1 else giorni
                                costo_totale = giorni * prezzo_giornaliero

                                # Aggiorna i campi della riga nel dataframe
                                df_agg.loc[i, COL_STATO] = "Noleggiata"
                                df_agg.loc[i, COL_CLIENTE] = cliente_nuovo.strip()
                                df_agg.loc[i, COL_DATA_INI] = str(data_inizio_noleggio)
                                df_agg.loc[i, COL_DATA_FIN] = str(data_fine_noleggio)
                                df_agg.loc[i, COL_COSTO] = float(costo_totale)
                                if note_noleggio.strip():
                                    df_agg.loc[i, COL_NOTE] = note_noleggio.strip()
                                ...
                                if res.status_code == 200 and res_json.get("status") in ["ok", "success"]:
                                    st.success(f"✅ Noleggio avviato con successo per il veicolo {targa_scelta}!")
                                    st.cache_data.clear()
                                    time.sleep(1)
                                    st.rerun()
                                else:
                            ...
                            st.error(f"Errore durante la registrazione del noleggio: {e}")
    else:
