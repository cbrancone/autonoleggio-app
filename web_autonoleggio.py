...
# ==========================================
# SUPABASE REST API (NESSUNA LIBRERIA ESTERNA)
# ==========================================
...
def get_veicoli(solo_disponibili=False):
    params = {"select": "*"}
    if solo_disponibili:
        params["stato"] = "eq.Disponibile"
    data = supabase_get("veicoli", params)
    return pd.DataFrame(data) if data else pd.DataFrame()


def inserisci_veicolo(targa, marca, modello, categoria, prezzo, anno):
    targa_fmt = targa.upper()

    # Controlla se la targa esiste già
    check = supabase_get(
        "veicoli", {"select": "targa", "targa": f"eq.{targa_fmt}"}
    )
    if check:
        return False, f"Targa {targa_fmt} già presente nel sistema."
    ...
    if supabase_post("veicoli", payload):
        return True, "Veicolo aggiunto con successo!"
    return False, "Errore durante l'inserimento del veicolo."


def elimina_veicolo(targa):
    supabase_delete("veicoli", "targa", targa)


def reset_parco_veicoli():
    url = f"{SUPABASE_URL}/rest/v1/veicoli?targa=neq."
...
def crea_noleggio(targa, cliente, doc, data_inz, data_fn, giorni, totale):
    ...
    if supabase_post("noleggi", payload):
        supabase_patch("veicoli", {"stato": "Noleggiato"}, "targa", targa)


def chiudi_noleggio(noleggio_id, targa):
    supabase_patch("noleggi", {"stato": "Completato"}, "id", noleggio_id)
    supabase_patch("veicoli", {"stato": "Disponibile"}, "targa", targa)


def get_noleggi(stato="Attivo"):
    data = supabase_get("noleggi", {"select": "*", "stato": f"eq.{stato}"})
    return pd.DataFrame(data) if data else pd.DataFrame()


def get_tutti_noleggi():
    data = supabase_get("noleggi", {"select": "*", "order": "id.desc"})
    return pd.DataFrame(data) if data else pd.DataFrame()
...
if menu == "📊 Dashboard":
    ...
    noleggiati = (
        len(df_veicoli[df_veicoli["stato"] == "Noleggiato"])
        if not df_veicoli.empty
        else 0
    )
    incasso_totale = (
        df_tutti_noleggi["costo_totale"].sum()
        if not df_tutti_noleggi.empty
        else 0.0
    )
    ...
    with col_left:
        ...
        if not df_veicoli.empty:
            st.dataframe(
                df_veicoli[
                    ["targa", "marca", "modello", "categoria", "stato"]
                ],
    ...
    with col_right:
        ...
        if not df_noleggi_attivi.empty:
            st.dataframe(
                df_noleggi_attivi[
                    ["targa", "cliente_nome", "data_fine", "costo_totale"]
                ],
                use_container_width=True,
                hide_index=True,
            )
...
# ------------------------------------------
    ...
    tab1, tab2, tab3 = st.tabs(
        ["📋 Lista Veicoli", "➕ Aggiungi Veicolo", "⚠️ Reset Flotta"]
    )
    ...
    with tab1:
        ...
        if df_veicoli.empty:
            ...
            for idx, row in df_veicoli.iterrows():
                ...
                col3.write(f"{float(row['prezzo_giornaliero']):.2f} €/gg")
    ...
    with tab2:
        ...
        with st.form("form_nuovo_veicolo", clear_on_submit=True):
            ...
            prezzo = col_a.number_input(
                "Prezzo Giornaliero (€)", min_value=10.0, value=50.0, step=5.0
            )
            anno = col_b.number_input(
            ...
            if submit:
                if targa and marca and modello:
                    esito, messaggio = inserisci_veicolo(
    ...
    with tab3:
        ...
        st.warning(
            "Attenzione: questa operazione eliminerà TUTTI i veicoli dal database."
        )

        conferma = st.checkbox(
            "Confermo di voler cancellare l'intero parco veicoli"
        )
        if st.button(
            "🚨 Svuota Intero Parco Veicoli",
            type="primary",
...
# ------------------------------------------
    ...
    if df_disponibili.empty:
        ...
        scelta_auto_str = st.selectbox(
            "Seleziona Veicolo Disponibile:", list(opzioni_auto.keys())
        )
        ...
        with st.form("form_noleggio"):
            ...
            data_inizio = col1.date_input(
                "Data Inizio Noleggio", datetime.date.today()
            )
            ...
            if giorni <= 0:
                st.error(
                    "⚠️ La data di fine noleggio deve essere successiva alla data di inizio."
                )
                totale_preventivo = 0.0
            else:
                totale_preventivo = giorni * prezzo_unitario
                st.info(
                    f"📊 **Riepilogo Costi:** {giorni} giorno/i x {prezzo_unitario:.2f} € = **{totale_preventivo:.2f} €**"
                )
            ...
            if btn_conferma:
                if giorni <= 0:
                    ...
                    st.success(
                        f"🎉 Noleggio registrato con successo per {cliente_nome}!"
                    )
                    st.balloons()
                    st.rerun()
                else:
                    st.error(
                        "Inserire tutti i dati del cliente prima di confermare."
                    )
...
# ------------------------------------------
    ...
    st.markdown(
        "Chiudi un noleggio attivo e ripristina la disponibilità del veicolo."
    )
    ...
    if df_attivi.empty:
        ...
        scelta_rientro = st.selectbox(
            "Seleziona Noleggio da Chiudere:", list(opzioni_rientro.keys())
        )
        ...
        col_det2.write(
            f"**Periodo:** dal {noleggio_sel['data_inizio']} al {noleggio_sel['data_fine']}"
        )
        col_det2.write(
            f"**Totale Noleggio:** {float(noleggio_sel['costo_totale']):.2f} €"
        )

        if st.button("✅ Registra Rientro Veicolo", type="primary"):
            chiudi_noleggio(noleggio_sel["id"], noleggio_sel["targa"])
            st.success(
                f"Auto targa {noleggio_sel['targa']} restituita con successo ed è ora nuovamente disponibile."
            )
            st.rerun()
